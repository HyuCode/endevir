...
- 日付: 2026-07-17
- 関連: [ADR-004](../01-adr/04-trace-recording-cost.md)
...
1. **全イベント共通の `seq`（単調増加）+ `timestampUs`（マイクロ秒）**: ログ⇔ステップの双方向参照は `stepId` で行い、時刻はすべて同一時間軸に乗る
...
5. **保証境界はtestStartに記録**: `testMode`（`inProcess` / `userPath`）はテストの操作契約を表し、実行プラットフォームやランナー名とは分離する（[ADR-008](../01-adr/08-test-mode-boundary.md)）
...
生成物は手で編集しない。スキーマ変更時は `pnpm codegen` を再実行してコミットする。CIでは生成差分がないことを検証する。

## バージョニング

- `runStart` イベントが `schemaVersion`（現在 "1"）を持つ。読み手は未知バージョンを拒否ではなく警告として扱う
- v1系内の変更は**追加のみ**（フィールド削除・意味変更はメジャーバージョンを上げる）

...
技術上の重要な決定を番号付きで記録する。技術検証の結論はすべてADRとして残す。

> **スパイク検証コードについて**: M0スパイクの検証コード（各ADRの「検証コード」リンク先）は、M1以降で `packages/` へ昇格したのち削除された。当時のコードはgit履歴（M0完了時点のコミット `e03735d` 以前）を参照。S3のintegration_test（イベント駆動待機のデバイス検証）のみ examples/flutter_app に現存する。
